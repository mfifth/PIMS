...
export default class extends Controller {
      ...
      fetch(`/batches/search?query=${encodeURIComponent(q)}&batch_id=${this.batchIdValue}`)
        ...
        .then(data => {
          ...
          data.forEach(item => {
            ...
            li.textContent = `${item.product_name} (SKU: ${item.sku}, Unit Type: ${item.unit_type}) — ${item.location_name} (Qty: ${item.quantity})`
  ...
  addItem(item) {
    ...
    li.innerHTML = `
      ${item.product_name} (SKU: ${item.sku}, Unit Type: ${item.unit_type}) — ${item.location_name} (Qty: ${item.quantity})
      <button type="button"
